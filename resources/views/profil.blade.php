...
@section('content')
<!-- PROFILE -->
@if(isset($formatedResponse))
        <div class="row">
           <div class="col s12 m8 offset-m2">
               <div id="alert_box" class="card green scale-transition scale-out">
                    <div class="card-content white-text">
                      <span class="card-title white-text darken-1 notifi">
                        <i class="material-icons">notifications_active</i> {{$formatedResponse['Response'] == 'Approved' ? 'Transakcija je uspešno izvršena.' : ''}}</span>
         ...
         <div id="profile" class="col tab-content user-data">
            <div id="profile-page-header" class="card">
               <div class="card-image waves-effect waves-block waves-light" >
                  <img class="activator bg -v" src="{{asset('/images/avatar/user-bg.png')}}" alt="user background">
               </div>
               <figure class="card-profile-image">
                 <!-- <img src="{{asset('/images/avatar/avatar-7.png')}}" alt="profile image" class="circle z-depth-2 responsive-img activator gradient-45deg-light-blue-cyan gradient-shadow">-->
                   <h2 @click="userMap();" class="circle z-depth-2 responsive-img activator gradient-45deg-light-blue-cyan gradient-shadow bg-v">{{substr(Auth::user()->first_name, 0, 1)}}{{substr(Auth::user()->last_name, 0, 1)}}</h2>
               </figure>
               <div class="card-content">
                  <div class="row pt-2">
                     <div class="col s12 m3 offset-m2">
                        <h4 class="card-title grey-text text-darken-4" > {{ Auth::user()->first_name }} {{ Auth::user()->last_name }}</h4>
                     ...
                     <div class="col s12 m3 center-align">
                        <h4 class="card-title grey-text text-darken-4"> <i class="material-icons top">add_shopping_cart</i> {{$amount}}</h4>
                        <p class="medium-small grey-text">Proizvoda Kupljeno</p>
                     </div>
                     <div class="col s12 m3 center-align">
                        <h4 class="card-title grey-text text-darken-4">{{$sum}} Din</h4>
               ...
               <div class="card-reveal">
                  ...
                  <div class="col s12 m5">
                     <p>
                        <span class="card-title grey-text text-darken-4"> {{ Auth::user()->first_name }} {{ Auth::user()->last_name }}
                        <i class="material-icons right">close</i>
                        </span>
                     </p>
                     <p>
                        <i class="material-icons font-violet">perm_phone_msg</i> {{ Auth::user()->phone }}
                     </p>
                     <p>
                        <i class="material-icons font-violet">email</i> {{ Auth::user()->email }}
                     </p>
                     <p>
                        <i class="material-icons font-violet">location_on</i> {{ Auth::user()->address }}
                     </p>
                     <p>
                        <i class="material-icons font-violet">language</i> {{ Auth::user()->country }}, {{ Auth::user()->city }}, {{ Auth::user()->zip }}
                     </p>
                  </div>
                     ...
                     <table class="responsive-table highlight bordered">
                        ...
                        <tbody>
                           ...
                           @foreach ($allInBaskets as $basket)
                           	@if($basket->response != 'Approved')
                           <tr @click="showInvoice({{$basket}},{{$allInBaskets}})">
                              <td colspan="2"> {{$basket->ItemInBasket[0]->name}}</td>
                              <td>{{$basket->ItemInBasket[0]->price}} Din</td>
                              <td>{{$basket->amount}} {{$basket->ItemInBasket[0]->amount}}</td>
                           ...
                           <tr  @click="showInvoice({{$basket}},{{$allInBaskets}})">
                              ...
                              <td>{{$basket->ItemInBasket[0]->price}} Din</td>
                                <td>{{$basket->amount}} {{$basket->ItemInBasket[0]->amount}}</td>
                              <td>{{$basket->created_at}}</td>
                                      <td><span class="task-cat green lighten-1">{{$basket->response == 'Approved' ? 'Odobreno' : $basket->response}}</span></td>
                           </tr>
                           	@endif
                           @endforeach
                           @endif
                              ...
                              <div class="invoice-header">
                                  <div class="row section">
                                    ...
                                    <h5 class="grey-text text-darken-4">Adresa plaćanja</h5>
                                        ...
                                        <p class="name m-bt"><span class="strong">Ime:</span> {{ Auth::user()->first_name }} {{ Auth::user()->last_name }}</p>
                                        <p class="address m-bt"><span class="strong">Adresa:</span> {{ Auth::user()->address }}</p>
                                          <p class="city m-bt"><span class="strong">Grad:</span> {{ Auth::user()->city }}</p>
                                          <p class="postal_code m-bt"><span class="strong">Poštanski kod:</span> {{ Auth::user()->zip }}</p>
                                          <p class="country m-bt"><span class="strong">Zemlja:</span> {{ Auth::user()->country }}</p>
                                          <p class="phone m-bt"><span class="strong">Tel:</span> {{ Auth::user()->phone }}</p>
                                      <div class="invoce-company-contact">
                                        <span class="invoice-icon">
                                          <i class="material-icons font-violet">mail_outline</i>
                                        </span>
                                        <p class="m-bt">
                                          <span class="mail"><span class="strong">E-mail:</span> {{ Auth::user()->email }}</span>
                                        </p>
                                      </div>
                                    </div>
                                     <div class="col s12 m4 l4 left-align">
...
<table class="responsive-table highlight bordered">
                        ...
                        <tbody>
                           <tr v-for="(invItem, index) in invItems" :index="index">
                              ...
                              <td v-if="(invItem.response == 'Canceled')"><span class="task-cat orange lighten-1">Poništeno</span></td>
                                    ...
                                    <form id="direct-form" action="{{ route('unpaid') }}" method="POST" style="display: none;">
                                           {{ csrf_field() }}
                ...
                <a href="http://kreativnihobi.bgsvetionik.com/password/reset">Promeni</a>
                     <form class="col s12" id="changePassword" name="changePassword"  method="POST" action="{{ route('password.request') }}">
                        {{ csrf_field() }}
